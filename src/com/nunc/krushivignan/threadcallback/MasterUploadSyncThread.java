...
public class MasterUploadSyncThread {
	private static final int sync = 1;
	private DBHelper db;
	 private ProgressDialog _dialog;
	private Context context;

	public MasterUploadSyncThread(Context context) {
	...
	public void startSync() {
		...
		Log.e("The responce data to server is", ServerURL.getSyncURL());
	}
...
//					}
//				});
				db.clearSyncData(AppInfo.CurrentUser);
				String farmer = ServerURL.getFarmerURL(AppInfo.CurrentUser
						.getUserId());
				String recommendation = ServerURL.getRecommendationURL(
						AppInfo.CurrentUser.getDistrict(),
						AppInfo.CurrentUser.getTaluk());
				String villageURL = ServerURL.getVillageURL(AppInfo.CurrentUser
						.getUserId());
				String soilMappingURL = ServerURL
						.getSoilMappingURL(AppInfo.CurrentUser.getDistrict());
				MasterDownloadSyncThread thread = new MasterDownloadSyncThread(
						context, farmer, recommendation, soilMappingURL,
						villageURL) {
					@Override
						...
						((Activity) context).runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								try {
									...
									// _editor.putBoolean("SyncStatus", true);
									// _editor.commit();
				...
				((Activity) context).runOnUiThread(new Runnable() {
					...
					public void run() {
						// TODO Auto-generated method stub
						_dialog = ProgressDialog.show(context, "",
								"Fetching Farmers & Farms....", false, false);
					}
				});

				Log.i("Sync Complete", "Sync Complete");
				break;
			}
		}
	};
